            ...
            <div className="min-h-screen bg-[#F4F6F5] flex items-top justify-center gap-3 w-full">
                <div className="w-full rounded-3xl bg-white shadow-sm p-6">
                    <div className="flex items-start justify-between mb-2">
                        <div>
                            <h2 className="text-xl font-bold">Checkout – Secure Your Purchase</h2>
                            <p className="text-sm text-gray-500 my-1">Fast, Secure, and Hassle-Free Transactions</p>
                        </div>
                        <button className="rounded-full p-1 hover:bg-gray-100" onClick={() => router.back()}>
                            <Image src={close} alt="Close" width={20} height={20} />
                        </button>
                    </div>

                    <section className="mb-6">
                        <h3 className="mb-3 text-base font-bold text-gray-900">Order Summary</h3>
                        <div className="space-y-2 text-sm">
                            <Row label="Item" value={post.title} />
                            <Row label="Type" value={post.type} />
                            <Row label="Seller" value={post.seller.name} />
                            <Row label="Price" value={`₹${post.price.toLocaleString()}`} valueClass="text-green-600 font-semibold" />
                        </div>
                    </section>
                    ...
                    <section>
                        <h3 className="mb-3 text-base font-bold text-gray-900">Buyer Details</h3>
                        <div className="space-y-2 text-sm">
...
const CheckoutPage = () => {
    return (
        <Suspense fallback={<SkeletonCheckoutLoader />}>
            <CheckoutContent />
        </Suspense>
    )
}
